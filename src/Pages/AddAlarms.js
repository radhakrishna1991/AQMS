...
function AddAlarms() {
  ...
  const Alarmaddvalidation = function (
    DriverEntryName,
    DriverInstrumentID,
    CoilNumber,
    Flag
  ...
  const Alarmadd = async function () {
    ...
    let DriverInstrumentID = document.getElementById(
    ...
    await fetch(CommonFunctions.getWebApiUrl() + "api/Alarm", {
      ...
      body: JSON.stringify({
        ...
        DeviceModelId: DriverInstrumentID,
        InputIndex: CoilNumber,
        Flag: Flag,
        CreatedBy: CreatedBy,
      }),
    })
      ...
      .then((responseJson) => {
        if (responseJson == "Alarmadd") {
          ...
          toast.error(
            "Alarm already exist with given Driver Name. Please try with another Alarm Name."
          );
        } else {
    ...
    await fetch(CommonFunctions.getWebApiUrl() + "api/Alarm/" + Driverid, {
      ...
      .then((responseJson) => {
        if (responseJson == 1) {
          ...
          toast.error(
            "Alarm already exist with given Alarm Name. Please try with another Alarm Name."
          );
        } else {
          toast.error(
            "Unable to update the Alarm. Please contact adminstrator"
          );
        }
      })
      .catch((error) =>
        ...
        await fetch(CommonFunctions.getWebApiUrl() + "api/Alarm/" + id, {
          ...
          .then((responseJson) => {
            if (responseJson == 1) {
              ...
              toast.error(
                "Unable to delete Alarm. Please contact adminstrator"
              );
            }
          })
          .catch((error) =>
  ...
  const GetLookupdata = async function () {
    ...
    await fetch(CommonFunctions.getWebApiUrl() + "api/AlarmsLookup", {
      ...
      .catch((error) =>
        toast.error(
          "Unable to get the Alarms lookup list. Please contact adminstrator"
        )
      )
      .finally(() => {
        setgridLoad(true);
  ...
  const GetAlarms = async function () {
    ...
    await fetch(CommonFunctions.getWebApiUrl() + "api/Alarm", {
      ...
      .catch((error) =>
        toast.error("Unable to get the alarm list. Please contact adminstrator")
      )
      .finally(() => {
        setgridLoad(true);
        document.getElementById("loader").style.display = "none";
          ...
          return $.grep(this.data, function (item) {
            ...
            return (
              (!filter.alarmName ||
                item.alarmName
                  .toUpperCase()
                  .indexOf(filter.alarmName.toUpperCase()) >= 0) &&
              (!filter.deviceModelName ||
                item.deviceModelName
                  .toUpperCase()
                  .indexOf(filter.deviceModelName.toUpperCase()) >= 0) &&
              (!filter.coilNumber ||
                (item.coilNumber == null
                  ? false
                  : item.coilNumber.toString().indexOf(filter.coilNumber) >= 0))
            );
          });
        },
      },
  ...
  const DownloadExcel = async function (filetype) {
    ...
    let authHeader = await CommonFunctions.getAuthHeader();
    await fetch(
      CommonFunctions.getWebApiUrl() + "api/AlarmsExportToExcel?" + params,
      {
        method: "GET",
        headers: authHeader,
      }
