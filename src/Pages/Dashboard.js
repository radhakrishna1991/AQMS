            ...
            for (var i = 0; i < data.listPollutents.length; i++) {
              ...
              for (let j = 0; j < intervalarr.length; j++) {
                ...
                let index = finalinterval.findIndex(
                  (x) =>
                    x.value === intervalsplitarr[0] &&
                    x.type === intervalsplitarr[1]
                );
                if (index == -1) {
                  finalinterval.push({
              ...
              if (
                Cookies.get(data.listPollutents[i].id + "_ChartStatus") !=
                "false"
              ) {
                checkedcnt++;
                Cookies.set(data.listPollutents[i].id + "_ChartStatus", true, {
                  expires: 7,
                });
                parameterChartStatus.push({
                  paramaterID: data.listPollutents[i].id,
                  paramaterName: data.listPollutents[i].parameterName,
                  ChartStatus: true,
                });
              } else {
                parameterChartStatus.push({
...
useEffect(() => {
  ...
  const Deviceinfo = function (param) {
    ...
    let parameters = ListAllData.listPollutents.filter(
      (x) => x.deviceID == param.id
    );
  ...
  const Devicealert = function (param) {
    ...
    let alarms = ListAllData.listAlarms.filter(
      (x) => x.deviceModelId == param.deviceModel
    );
    setInfoParameters(parameters);
  ...
  const DeviceGraphold = function (param, data) {
    ...
    for (var i = 0; i < LiveChartStatus.length; i++) {
      if (
        data.parameterName == LiveChartStatus[i].paramaterName &&
        data.id == LiveChartStatus[i].paramaterID
      ) {
  ...
  const DeviceGraph = function (data) {
    ...
    for (var k = 0; k < LiveChartStatus.length; k++) {
      if (
        Cookies.get(LiveChartStatus[k].paramaterID + "_ChartStatus") == "true"
      ) {
        checkedCount++;
      }
    }
  ...
  const Devicecalibration = async function (param) {
    ...
    let listcommands = ListAllData.listCommands.filter(
      (x) => x.deviceModelId == param.deviceModel
    );
    ...
    await fetch(
      ...
      .then((data) => {
        if (data) {
          ...
          let listcalibration = data.filter(
            (x) => x.parameterId == parameters[0].id
          );
  ...
  const GenerateChart = function (data) {
    ...
    for (let i = 0; i < pollutents.length; i++) {
      if (Cookies.get(pollutents[i].id + "_ChartStatus") == "true") {
        ...
        datasets.push({
          label: pollutents[i].parameterName,
          data: chartdata,
  ...
  const SaveCalibrationSequence = async function () {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      if (
        parseInt(data.FallingTime) + parseInt(data.RisingTime) >
        parseInt(data.TotalTime)
      ) {
        toast.error(
          "Total Time should be less than or equal to the sum of Raising Time and Falling Time"
        );
        return false;
      }
  ...
  const Parameterchange = function (e) {
    let listcalibration = CalibrationSequence.filter(
      (x) => x.parameterId == e.target.value
    );
    Setformvalues(listcalibration);
  };
  const Getformvalues = function (param) {
    ...
    let startdatetime = window
      .moment(StartDatetime1)
      .format("YYYY-MM-DD HH:mm:ss");
    if (param == 2) {
      startdatetime = window
        .moment(StartDatetime2)
        .format("YYYY-MM-DD HH:mm:ss");
    } else if (param == 3) {
      startdatetime = window
        .moment(StartDatetime3)
        .format("YYYY-MM-DD HH:mm:ss");
    } else if (param == 4) {
      startdatetime = window
        .moment(StartDatetime4)
        .format("YYYY-MM-DD HH:mm:ss");
    } else if (param == 5) {
      startdatetime = window
        .moment(StartDatetime4)
    ...
    let finalobject = {
      ...
      ProfileId: profile,
      SequenceNumber: param,
      RepeatedInterval: repeatedinterval,
      RepeatedIntervalType: repeatedintervaltype,
      ...
      ID: recid,
      Status: status,
      listCalibrationCommandsSequence: CommandsArray,
    };
    return finalobject;
  };
  ...
  const Setformvalues = function (list) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      let starttime =
        list[i].startTime == null ? new Date() : list[i].startTime;
      if (param == 1) {
        onChange1(starttime);
      } else if (param == 2) {
        onChange2(starttime);
      ...
      for (var j = 0; j < listcommands.length; j++) {
        $("#command" + param + listcommands[j].commandNumber).val(
          listcommands[j].command + "_" + listcommands[j].type
        );
      }
  ...
  const selects = function (data) {
    ...
    if (ele.checked == true) {
      for (var i = 0; i < LiveChartStatus.length; i++) {
        Cookies.set(LiveChartStatus[i].paramaterID + "_ChartStatus", true, {
          expires: 7,
        });
        LiveChartStatus[i].ChartStatus = true;
        var checkallvalues = document.getElementById(
          LiveChartStatus[i].paramaterID
        );
        checkallvalues.checked = true;
      }
    } else {
      for (var i = 0; i < LiveChartStatus.length; i++) {
        Cookies.set(LiveChartStatus[i].paramaterID + "_ChartStatus", false, {
          expires: 7,
        });
        LiveChartStatus[i].ChartStatus = false;
        var checkallvalues = document.getElementById(
    ...
    await fetch(
      ...
      .then((responseJson) => {
        if (responseJson == 1) {
          ...
          ListAllDataCopy.current.listPollutents
            .filter((x) => x.deviceID == param.id)
            .forEach(
              (x) => (x.flag = !x.isEnable ? null : param.serviceMode ? 5 : 1)
            );
        } else {
          toast.error(
  ...
  const GetTotalTime = function () {
    for (let i = 1; i <= 5; i++) {
      ...
      var addValue =
        Number(totaltime) - (Number(raisingtime) + Number(fallingTime));
                  ...
                  {Infodevices && (
                    <tbody>
                      ...
                      <tr className="body_active">
                        ...
                        <td>Type</td>
                        <td>
                          {Infodevices.type == "Serial" ? "Serial" : "Modbus"}
                        </td>
                      </tr>
              ...
              <div className="table-responsive">
                <table className="table align-middle table-bordered">
                  ...
                  {Infodevices && (
                    <tbody>
                      <tr>
                        <td rowSpan={Infoalarms.length + 1}>
                          {Infodevices.deviceName}
                        </td>
                      </tr>
                      {Infoalarms.map((i, j) => (
                      ...
                      {InfoParameters.map((x, y) => (
                        <React.Fragment>
                          <tr>
                            <td
                              rowSpan={
                                ListAllData.parameterAlarmsList.filter(
                                  (z) => z.parameterID == x.id
                                ).length + 1
                              }
                            >
                              {x.parameterName}
                          ...
                          {ListAllData.parameterAlarmsList
                            .filter((z) => z.parameterID == x.id)
                            .map((k, l) => (
                              <React.Fragment>
                                {!k.status == 1 && (
                                  <tr>
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <div className="calibrationmodal">
                ...
                >
                  <li className="nav-item" role="presentation">
                    <button
                      ...
                      id="sequence1-tab"
                      data-bs-toggle="tab"
                      data-bs-target="#sequence1-tab-pane"
                      type="button"
                      role="tab"
                      aria-controls="sequence1-tab-pane"
                      aria-selected="true"
                    >
                      Sequence1
                    </button>
                  ...
                  <li className="nav-item" role="presentation">
                    <button
                      ...
                      id="sequence2-tab"
                      data-bs-toggle="tab"
                      data-bs-target="#sequence2-tab-pane"
                      type="button"
                      role="tab"
                      aria-controls="sequence2-tab-pane"
                      aria-selected="false"
                    >
                      Sequence2
                    </button>
                  ...
                  <li className="nav-item" role="presentation">
                    <button
                      ...
                      id="sequence3-tab"
                      data-bs-toggle="tab"
                      data-bs-target="#sequence3-tab-pane"
                      type="button"
                      role="tab"
                      aria-controls="sequence3-tab-pane"
                      aria-selected="false"
                    >
                      Sequence3
                    </button>
                  ...
                  <li className="nav-item" role="presentation">
                    <button
                      ...
                      id="sequence4-tab"
                      data-bs-toggle="tab"
                      data-bs-target="#sequence4-tab-pane"
                      type="button"
                      role="tab"
                      aria-controls="sequence4-tab-pane"
                      aria-selected="false"
                    >
                      Sequence4
                    </button>
                  ...
                  <li className="nav-item" role="presentation">
                    <button
                      ...
                      id="sequence5-tab"
                      data-bs-toggle="tab"
                      data-bs-target="#sequence5-tab-pane"
                      type="button"
                      role="tab"
                      aria-controls="sequence5-tab-pane"
                      aria-selected="false"
                    >
                      Sequence5
                    </button>
                ...
                <div
                  className={
                    "tab-content " + (UserRole ? "" : "sequencedisable")
                  }
                  id="calibrationTabContent"
                >
                  {(() => {
                    let calibration = [];
                    for (let i = 1; i <= 5; i++) {
                      calibration.push(
                        <div
                          ...
                          id={"sequence" + i + "-tab-pane"}
                          role="tabpanel"
                          aria-labelledby={"sequence" + i + "-tab"}
                        >
                          <form id={"Sequenceform" + i} noValidate>
                                        ...
                                        <div className="col-md-8">
                                          ...
                                          >
                                            <option value="" selected>
                                              Select Type of the sequence
                                            </option>
                                            {window.Typeofsequence.map(
                                              (x, y) => (
                                      ...
                                      <div className="row mb-3">
                                        ...
                                        <div className="col-md-8 repeatedinterval">
                                          <input
                                            ...
                                            id={"repeatedinterval" + i}
                                            placeholder="Enter Interval"
                                            required
                                          />
                                          <select
                                            className="form-select"
                                            id={"repeatedintervaltype" + i}
                                          >
                            ...
                            <div className="float-end ">
                              <button
                                type="button"
                                className={
                                  "btn btn-primary px-3 py-1 my-1 " +
                                  (UserRole ? "" : "disable")
                                }
                                onClick={() => Clearformvalues(i)}
                              >
                                Clear
                              </button>
                            </div>
            ...
            <div className="modal-footer">
              ...
              <button
                type="button"
                className={"btn btn-primary " + (UserRole ? "" : "disable")}
                onClick={SaveCalibrationSequence}
              >
                Save
              </button>
            </div>
                          ...
                          <div className="card-body ">
                            ...
                            <div className="d-flex justify-content-start mt-2">
                              ...
                              {ListAllData.listAlarms.filter(
                                (z) =>
                                  z.deviceModelId == x.deviceModel &&
                                  z.status == 1
                              ).length > 0 && (
                                <div
                                  className="icons blink"
                                  title="Alert"
                                  onClick={() => Devicealert(x)}
                                >
                            ...
                            {ListAllData.listPollutents.map(
                              (i, j) =>
                                i.deviceID == x.id &&
                                i.status == 1 && (
                                  <div className="d-flex justify-content-between mt-2">
                                    <div className="parameter">
                                      <span onClick={() => ParameterEnable(i)}>
                                        {i.isEnable && (
                                          <i
                                            className={
                                              "bi bi-check2 " +
                                      ...
                                      <span>{i.parameterName}</span>
                                    </div>
                                    {i.parameterName.toUpperCase() == "RAIN" &&
                                    i.parseFunction == "" ? (
                                      <div className="values">
                                        <button
                                          ...
                                          style={{
                                            backgroundColor:
                                              i.flag != null
                                                ? ListAllData.listFlagCodes.filter(
                                                    (y) => y.id == i.flag
                                                  )[0].colorCode
                                                : "#FFFFFF",
                                          }}
                                        ...
                                        <button className="btn2">
                                          {ListAllData.rainLiveValue == null
                                            ? "-"
                                            : ListAllData.rainLiveValue.toFixed(
                                                window.DashboardLivenumberround
                                              )}
                                        </button>
                                        &nbsp;
                                        <sub>
                                          {ListAllData.listReportedUnits.filter(
                                            (x) => x.id === i.unitID
                                          ).length > 0
                                            ? ListAllData.listReportedUnits
                                                .filter(
                                                  (x) => x.id === i.unitID
                                                )[0]
                                      ...
                                      <div className="values">
                                        ...
                                        <button className="btn2">
                                          {i.parameterValue == null
                                            ? "-"
                                            : i.parameterValue.toFixed(
                                                window.DashboardLivenumberround
                                              )}
                                        </button>
                                        &nbsp;
