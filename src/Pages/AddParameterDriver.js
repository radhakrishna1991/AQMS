...
function AddParameterTemplates() {
  ...
  const ParameterDriveradd = async function () {
    ...
    let associatedinstrument = document.getElementById(
      "associatedinstrument"
    ).value;
    let DataType = document.getElementById("DataType").value;
    let DataFormat = document.getElementById("DataFormat").value;
    let ModusRegisterIndex =
      document.getElementById("ModusRegisterIndex").value;
    ...
    await fetch(CommonFunctions.getWebApiUrl() + "api/ParameterDriver", {
      ...
      body: JSON.stringify({
        DriverName: ParameterDriver,
        DeviceModelID: associatedinstrument,
        DataType: DataType,
        DataFormat: DataFormat,
        RegisterIndex: ModusRegisterIndex,
        SendCommand: SendCommand,
        SendInterval: finalInterval,
      ...
      .then((responseJson) => {
        if (responseJson == "Parameterdriveradded") {
          ...
          toast.error(
            "Parameter Driver already exist with given Parameter Driver Name. Please try with another Parameter Driver Name."
          );
        } else {
          toast.error(
            "Unable to add the Parameter Driver. Please contact adminstrator"
          );
        }
      })
      .catch((error) =>
  ...
  const EditParameterDriver = function (param) {
    ...
    setTimeout(() => {
      ...
      setValue((prevDisplay) => ({
        ...prevDisplay,
        ["ParameterDriverid"]: param.driverName,
      }));
      document.getElementById("associatedinstrument").value =
        param.deviceModelID;
      ...
      setValue((prevDisplay) => ({
        ...prevDisplay,
        ["ModusRegisterIndexid"]: param.registerIndex,
      }));
      ...
      let SendInterval =
        param.sendInterval != null ? param.sendInterval.split("-") : "";
      document.getElementById("SendInterval").value =
        SendInterval == "" ? "" : SendInterval[0];
      setValue((prevDisplay) => ({
        ...prevDisplay,
        ["SendIntervalid"]: SendInterval == "" ? "" : SendInterval[0],
      }));
      document.getElementById("SendInterval1").value =
        SendInterval == "" ? "" : SendInterval[1];
      document.getElementById("ParseFunction").value = param.parseFunction;
      setValue((prevDisplay) => ({
        ...prevDisplay,
        ["ParseFunctionid"]: param.parseFunction,
      }));
    }, 10);
  };
  ...
  const UpdateParameterDriver = async function () {
    ...
    let authHeader = await CommonFunctions.getAuthHeader();
    await fetch(
      CommonFunctions.getWebApiUrl() + "api/ParameterDriver/" + Instrumentid,
      {
        method: "PUT",
        headers: {
          Accept: "application/json",
      ...
      .then((responseJson) => {
        if (responseJson == 1) {
          ...
          toast.error(
            "Unable to update the Parameter Driver. Please contact adminstrator"
          );
        }
      })
      .catch((error) =>
  ...
  const DeleteParameterDriver = function (item) {
    Swal.fire({
      ...
      if (isConfirm.isConfirmed) {
        ...
        let authHeader = await CommonFunctions.getAuthHeader();
        await fetch(
          CommonFunctions.getWebApiUrl() + "api/ParameterDriver/" + id,
          {
            method: "DELETE",
            headers: authHeader,
          }
          ...
          .then((responseJson) => {
            if (responseJson == 1) {
              ...
              toast.error(
                "Unable to delete Parameter Driver. Please contact adminstrator"
              );
            }
          })
          .catch((error) =>
  ...
  const GetLookupdata = async function () {
    ...
    let authHeader = await CommonFunctions.getAuthHeader();
    await fetch(
      CommonFunctions.getWebApiUrl() + "api/ParametersTemplateslookup",
      {
        method: "GET",
        headers: authHeader,
      }
      ...
      .catch((error) =>
        toast.error(
          "Unable to get the lookup list. Please contact adminstrator"
        )
      )
      .finally(() => {
        setgridLoad(true);
  ...
  const GetParameterDrivers = async function () {
    ...
    await fetch(CommonFunctions.getWebApiUrl() + "api/ParameterDriver", {
      ...
      .catch((error) =>
        toast.error(
          "Unable to get the Parameter Drivers list. Please contact adminstrator"
        )
      )
      .finally(() => {
        setgridLoad(true);
          ...
          return $.grep(this.data, function (item) {
            return (
              (!filter.driverName ||
                item.driverName
                  .toUpperCase()
                  .indexOf(filter.driverName.toUpperCase()) >= 0) &&
              (!filter.deviceModelID ||
                item.deviceModelID === filter.deviceModelID) &&
              (!filter.registerIndex ||
                (item.registerIndex == null
                  ? false
                  : item.registerIndex
                      .toString()
                      .includes(filter.registerIndex)))
            );
          });
        },
      },
  ...
  const DownloadExcel = async function (filetype) {
    ...
    await fetch(
      CommonFunctions.getWebApiUrl() +
        "api/ParameterDriversExportToExcel?" +
        params,
      {
        method: "GET",
        headers: authHeader,
