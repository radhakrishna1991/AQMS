...
function AppLogHistory() {
  ...
  const GetAppLogLookup = async function () {
    ...
    await fetch(CommonFunctions.getWebApiUrl() + "api/AppLogHistory", {
      ...
      .then((data) => {
        if (data) {
          let data1 = data.listAppLogHistory.map((x) => {
            x.logTime = x.logTime.replace("T", " ");
            return x;
          });
          setListAppLog(data1);
        }
      ...
      .catch((error) =>
        toast.error(
          "Unable to get the application log list. Please contact adminstrator"
        )
      )
      .finally(() => {
        setgridLoad(true);
  ...
  const GetAppLog = async function (param) {
    ...
    if (param == "reset") {
      ...
      let authHeader = await CommonFunctions.getAuthHeader();
      await fetch(
        CommonFunctions.getWebApiUrl() + "api/AppLogHistoryByFilter?" + params,
        {
          method: "GET",
          headers: authHeader,
        }
      ...
      controller: {
        ...
        loadData: function (filter) {
          ...
          return $.grep(this.data, function (item) {
            // console.log(item);
            return (
              (!filter.logID || item.logID == filter.logID) &&
              (!filter.logDescription ||
                item.logDescription
                  .toUpperCase()
                  .indexOf(filter.logDescription.toUpperCase()) >= 0) &&
              (!filter.logSource ||
                item.logSource
                  .toUpperCase()
                  .indexOf(filter.logSource.toUpperCase()) >= 0) &&
              (!filter.logTime ||
                item.logTime
                  .toUpperCase()
                  .indexOf(filter.logTime.toUpperCase()) >= 0)
            );
          });
        },
      },
