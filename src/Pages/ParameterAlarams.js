...
function ParameterAlarams() {
  ...
  const initializeJsGrid = function () {
    ...
    ParameterAlarmData.filter(function (item) {
      var i = dataForGrid.findIndex(
        (x) => x.deviceID == item.deviceID && x.parameterID == item.parameterID
      );
      if (i <= -1) {
  ...
  const DeleteParameterAlarm = function (item) {
    Swal.fire({
      ...
      if (isConfirm.isConfirmed) {
        ...
        let authHeader = await CommonFunctions.getAuthHeader();
        await fetch(
          CommonFunctions.getWebApiUrl() + "api/DeleteParameterAlarm/" + id,
          {
            method: "DELETE",
            headers: authHeader,
          }
          ...
          .then((responseJson) => {
            if (responseJson == 1) {
              ...
              toast.error(
                "Unable to delete Parameter Alarm. Please contact adminstrator"
              );
            }
          })
          .catch((error) =>
  ...
  const ParameterAlarmaddvalidation = function (
    deviceid,
    parameterid,
    alarmid
  ) {
  ...
  const ParameterAlarmadd = async function () {
    ...
    var parameterArray = [];
    let validation = ParameterAlarmaddvalidation(
      deviceid,
      parameterid,
      alarmid
    );
  ...
  const UpdateParameterAlarm = async function () {
    ...
    let authHeader = await CommonFunctions.getAuthHeader();
    await fetch(
      CommonFunctions.getWebApiUrl() + "api/ParameterAlarm/" + ParameterAlarmId,
      {
        method: "PUT",
        headers: {
          Accept: "application/json",
      ...
      .then((responseJson) => {
        if (responseJson == 1) {
          ...
          toast.error(
            "Parameter Alarm already exist with given Parameter Name. Please try with another Parameter Name."
          );
        } else {
          toast.error(
            "Unable to update the Parameter Alarm. Please contact adminstrator"
          );
        }
      })
      .catch((error) =>
  ...
  const EditParameterAlarm = function (param) {
    ...
    let parameterAlarm = ParameterAlarmData.filter(
      (x) => x.deviceID == param.deviceID && x.parameterID == param.parameterID
    );
    let enable =
      ParameterAlarm.length > 0 && ParameterAlarmData[0].isEnable == 1
        ? true
        : false;
  ...
  const DownloadExcel = async function (filetype) {
    ...
    await fetch(
      CommonFunctions.getWebApiUrl() +
        "api/ParameterAlarmsListExportToExcel?" +
        params,
      {
        method: "GET",
        headers: authHeader,
      ...
      <div className="container">
        <div className="row my-2">
          <div className="pagetitle col">
            ...
            {ParameterAlarmList && <h1>Parameter Alarm List</h1>}
          </div>
          <div className="col text-end">
            {ParameterAlarmList ? (
              <span
                className="operation_class mx-2"
                onClick={() => AddParameterAlarmchange()}
              >
                <i className="bi bi-plus-circle-fill"></i>{" "}
                <span>Create New Parameter Alarm</span>
              </span>
            ) : (
              <span
                className="operation_class mx-2"
                onClick={() => AddParameterAlarmchange("gridlist")}
              >
                <i className="bi bi-card-list"></i>{" "}
                <span>View Parameter Alarms</span>
              </span>
            )}
          </div>
        </div>
              ...
              <>
                ...
                <form id="ParameterAlarmsform" className="row" noValidate>
                  ...
                  <div className="col-md-6 text-center mt-md-4 mb-3">
                    <label for="Enableid" className="form-label">
                      Enable:{" "}
                    </label>
                  ...
                  <div className="col-md-12 text-center">
                    ...
                    {!ParameterAlarmList && ParameterAlarmId != 0 && (
                      <button
                        className="btn btn-primary"
                        onClick={UpdateParameterAlarm}
                        type="button"
                      >
                        Update Parameter Alarm
                      </button>
                    )}
                  </div>
                </form>
        ...
        {ParameterAlarmList && Parameter.length > 0 && (
          <div align="center">
            <button
              type="button"
