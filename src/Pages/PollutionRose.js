...
function PollutionRose() {
  ...
  const ChangeStation = function (e) {
    ...
    let configuredParams = finaldata.filter((item) =>
      PollutionRoseConfig.ConfigList.some(
        (configParam) =>
          configParam.parametername === item.parameterName &&
          configParam.unit === item.unitName
      )
    );
    // setselectedStations(e.target.value);
    setPollutents(configuredParams);
  };
  ...
  const filterData = function (param, filteredData) {
    ...
    return filteredData?.filter(
      (obj) => obj.sp > parseInt(Paramsplit[0]) && obj.sp <= Paramsplit[1]
    );
  };
  ...
  const GenarateReport = async function () {
    ...
    await fetch(
      CommonFunctions.getWebApiUrl() +
        "api/AirQuality/getPollutionRose?" +
        params,
      {
        method: "GET",
        headers: authHeader,
      ...
      .then((data) => {
        if (data) {
          ...
          let count = 0;
          let finalData = [];
          let driverID = AllLookpdata.listPollutents.find(
            (x) =>
              x.parameterName?.toLowerCase() == PollutentName?.toLowerCase() &&
              x.stationID == StationID
          ).driverID;
          let unitID = AllLookpdata.listPollutents.find(
            (x) =>
              x.parameterName?.toLowerCase() == PollutentName?.toLowerCase() &&
              x.stationID == StationID
          ).unitID;
          let unitName = AllLookpdata.listReportedUnits.find(
            (x) => x.id == unitID
          ).unitName;
          let driverName = AllLookpdata.listDrivers.find(
            (x) => x.id == driverID
          ).driverName;
          // configparam=PollutionRoseConfig?.ConfigList?.filter(x=>x.parametername?.toLowerCase()==PollutentName?.toLowerCase());
          configparam = PollutionRoseConfig?.ConfigList?.filter(
            (x) =>
              x.parametername?.toLowerCase() == driverName?.toLowerCase() &&
              x.unit == unitName
          );
          ...
          for (let i = 0; i < Paramsplit?.length; i++) {
            ...
            datalabels.push(
              Paramsplit[i] + " " + configparam[0]?.unit?.toLowerCase()
            );
          }
